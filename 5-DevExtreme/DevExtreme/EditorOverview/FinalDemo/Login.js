            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (!response.isError && response.data) {
                        ...
                        // Check if the role key exists in the decoded token and assign default value if not found
                        let role = decodedToken.role || "User"; // Directly using `role` key without the URL-specific prefix
                        console.log("User Role:", role);
